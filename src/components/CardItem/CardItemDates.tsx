...
function CardItemDates(props: CardItemDatesProps) {
  ...
  return (
    <span
      ...
      className={` group flex relative items-center justify-center w-fit max-w-full h-6 mb-1 p-[2px] rounded-sm text-xs pr-1  ${
    ...
    >
      <span className=" group-hover:hidden flex items-center justify-center w-5 h-5 leading-5">
        <Clock strokeWidth={1.75} size={14} className=" inline-block" />
      </span>
      <span
        className={` hidden ${
          card.isComplete &&
          "group-hover:flex items-center justify-center w-5 h-5 leading-5"
        }`}
      >
